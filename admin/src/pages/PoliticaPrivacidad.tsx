...
function PoliticaPrivacidad() {
  ...
  useEffect(() => {
    ...
    const getInitialContent = async () => {
      try {
        const res = await axios.get(constants.API_URL + 'paginas/politicas-privacidad');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (content) {
        ...
        const res = await axios.put(constants.API_URL + 'paginas/politicas-privacidad/update', { contenido: content })
        if (res.data.status === 200) {
