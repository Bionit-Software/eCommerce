...
const TableMarcas = () => {
  ...
  const [idEliminar, setIdEliminar] = useState(0); // Estado para el id de la marca a eliminar
  ...
  const fetchmarcas = async (page: number) => {
    try {
      setLoading(true);
      const response = await axios.get(constants.API_URL + `marcas/pages/${page}?searchTerm=${searchTerm}`);
  ...
  const fetchData = async () => {
    try {
      const numeroPaginas = await axios.get(constants.API_URL + 'marcas/totalPages');
  ...
  const hadleOpenModal = (marca: any) => {
    if (open === false) {
      setOpen(true);
      setMarca(marca);
    } else {
  ...
  const handleCreate = (e: any, marca: any) => {
    e.preventDefault();
    if (marca.Nombre !== '' && marca.Nombre !== '') {
      axios.post(constants.API_URL + 'marcas/create', marca)
        .then((response) => {
  ...
  const handleEdit = (e: any, marca: any) => {
    e.preventDefault();
    axios.put(constants.API_URL + 'marcas/edit/' + marca.ID, marca)
      .then((response) => {
        if (response.data.status === 200) {
          toast.success(response.data.message);
          setOpen(false);
          ...
          <form className="flex flex-col gap-5">
            ...
            <div className="flex justify-between">
              ...
              <button
                ...
                onClick={(e) => handleCreate(e, marca)}
              >
                Guardar
              </button>
            </div>
        ...
        <div className="bg-white dark:bg-boxdark rounded-lg border border-stroke dark:border-strokedark w-96 mx-auto mt-20 p-5">
          ...
          <form className="flex flex-col gap-5">
            ...
            <div className="flex justify-between">
              <button
                className="dark:bg-stroke rounded-md py-2 px-4 mr-2 hover:bg-[#E9EAEC] dark:hover:bg-stroke bg-[#F4F5F7]"
                onClick={() => hadleOpenModal(marca)}
              >
                Cancelar
              </button>
              <button
                className="bg-[#F4F5F7] dark:bg-stroke rounded-md py-2 px-4 hover:bg-[#E9EAEC] dark:hover:bg-stroke"
                onClick={(e) => handleEdit(e, marca)}
              >
                Guardar
              </button>
            </div>
        ...
        <div className="bg-white dark:bg-boxdark rounded-lg border border-stroke dark:border-strokedark w-96 mx-auto mt-20 p-5">
          ...
          <div className="flex justify-between mt-5">
            ...
            <button
              className="bg-[#F4F5F7] dark:bg-stroke rounded-md py-2 px-4 hover:bg-[#E9EAEC] dark:hover:bg-stroke"
              onClick={() => {
                axios.delete(constants.API_URL + 'marcas/' + idEliminar)
                  .then((response) => {
                    if (response.data.status === 200) {
                      toast.success(response.data.message);
                      setOpenModalEliminar(false);
      ...
      <div className="max-h-115 md:max-h-125 lg:max-h-180 overflow-y-auto overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
                ...
                <td className="py-5 px-4 pl-9 xl:pl-11">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm">Tenés que agregar marcas</p>
                    </div>
                  </div>
                </td>
              </tr>
            ...
            {marcas !== undefined && marcas.map((marca: any) => (
              <tr key={marca.id} className="border-b border-[#eee] dark:border-strokedark">
                <td className="py-5 px-4 pl-9 xl:pl-11">
                  <div className="flex items-center">
                    <div className="w-10 h-10 flex-shrink-0 mr-3">
                      <img
                ...
                <td className="py-5 px-4 flex justify-center">
                  ...
                  <button
                    className="bg-[#F4F5F7] dark:bg-stroke rounded-md py-2 px-4"
                    onClick={() => hadleOpenModalEliminar(marca.ID)}
                  >
                    Eliminar
                  </button>
                </td>
