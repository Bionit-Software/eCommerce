...
const DropdownUser = () => {
  ...
  useEffect(() => {
    const fetchDolar = async () => {
      try {
        const res = await axios.get(constants.API_URL + 'dolar/1');
        console.log(res.data.data.valor);
        if (res.data.status === 200) {
          setDolarInput(res.data.data.valor);
        } else {
  ...
  // const handleDolarOnChange = async (e:any) => {
  //   setDolarInput(Number(e.target.value))
  //   console.log('click')
  //   console.log(dolarInput);
  //   const res = await axios.put(constants.API_URL + 'dolar/1', { valor: Number(e.target.value) });
  //   if (res.data.status === 200) {
  ...
  const handleDolarOnChange = async (e: any) => {
    clearTimeout(timerId); // Limpiar el temporizador anterior

    const inputValue = Number(e.target.value);
    setDolarInput(inputValue);

    timerId = setTimeout(async () => {
      console.log('Valor del dólar actualizado:', inputValue);
      const res = await axios.put(constants.API_URL + 'dolar/1', { valor: inputValue });
    }, 5500); // Esperar 1 segundo después del último cambio antes de hacer la solicitud
  };
    ...
    <div className="relative">
      ...
      >
        <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
          <li>
            <div className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base' onClick={handleLogout}>
              <svg
            ...
            <Input
              type="number"
              value={dolarInput}
              onChange={(e) => handleDolarOnChange(e)}
            />
          </li>
        </ul>
        <button
