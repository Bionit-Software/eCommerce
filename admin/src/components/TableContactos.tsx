...
const TableContactos = () => {
  ...
  const handleCreated = () => {
    ...
    fetchContactos(currentPage, estadoOrder, createdOrder);
  }
  ...
  const handleStock = (e: any) => {
    ...
    fetchContactos(currentPage, e.target.value, createdOrder);
  };

  const fetchContactos = async (page: number, estadoOrder: string, createdOrder: string) => {

    try {
      setLoading(true);
      const response = await axios.get(constants.API_URL + `paginas/contacto/pages/${page}?estadoOrder=${estadoOrder}&createdOrder=${createdOrder}`);
  ...
  const fetchData = async () => {
    try {
      const numeroPaginas = await axios.get(constants.API_URL + 'paginas/contacto/totalPages');
      console.log(numeroPaginas.data)
      setTotalPages(numeroPaginas.data);
      await fetchContactos(currentPage, estadoOrder, createdOrder);
    } catch (error) {
      console.error('Error fetching data:', error);
      // Handle the error or display a user-friendly message
    } finally {
  ...
  //       // Contactos.filter((contacto: any) => contacto.ID !== id);
  //       fetchData();
  //     })
  // }
      ...
      <div className="max-h-115 md:max-h-125 lg:max-h-180 overflow-y-auto overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            ...
            {contactos !== undefined && contactos.map((contacto: any) => (
              <tr key={contacto.id} className="border-b border-[#eee] dark:border-strokedark">
                ...
                <td className="py-5 px-4">
                  <div className="flex items-center space-x-3.5 justify-center">
                    <button
                      onClick={() => navigate('/contacto/view/' + contacto.id, { state: { contacto } })}
